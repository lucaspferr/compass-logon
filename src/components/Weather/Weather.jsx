...
export const Weather = () => {
    ...
    const getWeather = async () => {
      const response = await fetch(`http://api.weatherapi.com/v1/ip.json?key=${import.meta.env.VITE_API_KEY}&q=auto:ip`);
      const jsonBody = await response.json();
      setLocation(jsonBody.city);
      const response2 = await fetch(`http://api.weatherapi.com/v1/current.json?key=${import.meta.env.VITE_API_KEY}&q=${jsonBody.city}`);
